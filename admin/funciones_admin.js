...
function CambiarColorOut(el, bg) {
	if (el.style) {
		...
		el.style.backgroundColor = bg;
                el.children.tags('A')[0].style.color = '#003366';
	}
}
...
function valida_busqueda_sorteo_01(xforma) {
   ...
   if (!valida_campo_int(xforma.xnumsorteo,'Olvido introducir el N� del Sorteo','Introduzca un n�mero valido, Por Favor!')) return;
   xforma.submit();
...
function confirmar_submit_dbmelate(xform, mensaje) {
  if (!valida_campo_int(xform.xsorteo,'Olvido introducir el N. del Sorteo','Introduzca un número valido, Por Favor!')) return;  
  if (!valida_campo_int(xform.xnat1,'Olvido introducir el 1er. Número Natural','Introduzca un número valido, Por Favor!')) return;  
  if (!valida_campo_int(xform.xnat2,'Olvido introducir el 2er. Número Natural','Introduzca un número valido, Por Favor!')) return;  
  if (!valida_campo_int(xform.xnat3,'Olvido introducir el 3er. Número Natural','Introduzca un número valido, Por Favor!')) return;  
  if (!valida_campo_int(xform.xnat4,'Olvido introducir el 4er. Número Natural','Introduzca un número valido, Por Favor!')) return;  
  if (!valida_campo_int(xform.xnat5,'Olvido introducir el 5er. Número Natural','Introduzca un número valido, Por Favor!')) return;  
  if (!valida_campo_int(xform.xnat6,'Olvido introducir el 6er. Número Natural','Introduzca un número valido, Por Favor!')) return;  
  if (confirm(mensaje)) {
     xform.submit();
  }
